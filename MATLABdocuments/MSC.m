...
% 4. MSC 多元散射校正处理（Multivariate Scatter Correction）
% 4.1 计算平均光谱；Calculate the average spectrum
mean_spectrum = mean(spectra, 2); 

% 4.2 初始化一个空矩阵存放校正后的数据;Initialize an empty matrix to store the corrected data.
[n_points, n_samples] = size(spectra);
spectra_msc = zeros(n_points, n_samples);

% 4.3 循环处理每一个样本;Process each sample in a loop
for i = 1:n_samples
